...
$config = array(
	...
	'shanpay_partner'	=>	array(
		'INPUT_TYPE'	=>	'430111418452544',
	),
	'shanpay_key'	=>	array(
		'INPUT_TYPE'	=>	'Qtri5zAsGzFCp2X4aAcREyEvQhTBZUeU'
	)
);
...
class Shanpay_payment implements payment {
	...
	public function get_display_code()
	{
		...
		if($payment_item)
		{
			$html = "<label  rel='payment_rdo'>烽火支付：
<input  style='display: inline-block !important;' type='radio' name='payment' value='".$payment_item['id']."'/>";

			if($payment_item['logo']!='')
			{
				$html .= "<img style='position: absolute;width: 123px;height: 39px;margin-top: -12px;' src=".$payment_item['logo'].">";
			}
			else
			{
