<?php
	 ...
	 * @return string
	 */
	function RegisterCreditor($order_id,$t_user_id,$platformNo,$post_url){
...
<head>
	...
	<body>
		...
		<div style="width:100%;text-align:center;padding:50px 0;"><img src="'.APP_ROOT.'/app/Tpl/'.app_conf("TEMPLATE").'/images/loading.gif" />正在跳转，请稍后...</div>
	</body>
</html>
<script language="javascript">document.form1.submit();</script>';
		$yeepay_log = array();
	...
	function RegisterCreditorCallBack($str3Req){
		...
		if ($GLOBALS['db']->affected_rows()){	
			...
			if ($str3Req['code'] == '1'){
				...
				$GLOBALS['db']->query("update  ".DB_PREFIX."deal_order set online_pay=".$ipsdata['paymentAmount']." where order_status=0 and id=".$ipsdata['tenderOrderNo']);
 				...
 				save_log_common('-'.$ipsdata['paymentAmount'],$GLOBALS['user_info']['id'],$log_info,array('type'=>4,'deal_id'=>$order['deal_id']));
  				if($GLOBALS['db']->affected_rows()){
