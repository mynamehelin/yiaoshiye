...
require_once(APP_ROOT_PATH.'system/collocation/collocation.php');
class Yeepay_collocation implements collocation {
	...
	function __construct(){		
		...
		if ($collocation_cfg['is_debug'] == 1){
			$this->post_url="http://119.161.147.110:8088/member";
			$this->ws_url= "http://119.161.147.110:8088/member";
		}else{
	...
	function RegisterCreditor($order_id,$t_user_id){
		...
		return RegisterCreditor($order_id,$t_user_id,$this->platformNo, $this->post_url);
	}
	/*
	 * 接入懒猫
	 */
	function RegisterCat($t_user_id){
		
		require_once(APP_ROOT_PATH.'system/collocation/yeepay/RegisterCat.php');
		
		return RegisterCat($t_user_id,$this->platformNo, $this->post_url);
	}
	/**
	 * 登记债权转让
