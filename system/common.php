...
function pay_order($order_id)
{
  	...
  	if($GLOBALS['db']->affected_rows()>0) //订单已成功支付
	{
		...
		if($order_info['score']>0)
		{
			...
			modify_account(array("money"=>$order_info['score_money'],"score"=>"-".$order_info['score']),$order_info['user_id'],$log_score);
		}
		
  		$re=modify_account(array("money"=>"-".$order_info['total_price']),$order_info['user_id'],$order_info['deal_name']."购买成功");
   		if(!$re){
 			$result['status'] = 5;
		...
		if($GLOBALS['db']->affected_rows()>0)
		{
			...
			if($GLOBALS['db']->affected_rows()>0||($order_info['type']==1))
			{
				...
				if($point_multiple >0)
				{
					...
					$log_info=$order_info['deal_name']."购买成功,信息值增加".$point;
...
function syn_deal_status($deal_id)
{
	...
	{		
		...
		$GLOBALS['db']->query("update ".DB_PREFIX."deal_order set repay_make_time='".get_gmtime()."',repay_time='".get_gmtime()."',repay_memo='已认筹,已分红' where order_status=3 and is_refund=0 and type=2 ");
		//项目成功，加入项目成功的待发队列
...
/*weic 充值短信提醒 2017.3.15*/
function send_addMoney_sms($mobile,$money,$username)
{
if(app_conf("SMS_ON")==1)
	{
										
				$tmpl = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."msg_template where name = 'TPL_SMS_ADDMONEY_SUCCEE'");				
				$tmpl_content = $tmpl['content'];
				$verify['mobile'] = $mobile;
				$verify['money'] = $money;
