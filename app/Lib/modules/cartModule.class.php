...
class cartModule extends BaseModule
{
	...
	public function check()
	{
		...
		$id = intval($_REQUEST['id']);
		$deal_item = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_item where id = ".$id);
        $isnull=$GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_order where deal_id = ".$deal_item['deal_id']);
          foreach($isnull as $k)
          {
              $uname[]=$k['user_name'];
          }
...
/*************修改金额限制*******************************weic 2017.2.24******************************************************/
			if($deal_item['type']==1){
				$pay_money=floatval($_REQUEST['pay_money']);
				$done_money=floatval($_REQUEST['done_money']);
				$limit_money=floatval($_REQUEST['limit_money']);
				
				$maxMoney = $pay_money+$done_money;
				//$pay_money+$support_money > $limit_money
				//`showErr('"'.$maxMoney.'"',1);


            if(strpos($deal_info['name'],"型号C15") || strpos($deal_info['name'],"【七天体验标（5）】"))
				{

                   $as=$GLOBALS['user_info']["user_name"];
                     $ss=$deal_item['deal_id'];
					$deal_items = $GLOBALS['db']->getAll("select deal_price from ".DB_PREFIX."deal_order where user_name = '$as' And deal_id='$ss'");
					$umoney=0;
					foreach($deal_items as $k)
					{
						$umoney+=$k['deal_price'];
					}
                   $asd=$umoney+$pay_money;
					if($asd  > "10000")
					{
						showErr("您最多可以投资10000哦",1);
					}

					if($pay_money > "10000")
					{
						showErr("您最多可以投资10000",1);
					}
                ...
                if($maxMoney <= $limit_money){
                    showSuccess("",$ajax,url("cart#pay",array("id"=>$id,'pay_money'=>$pay_money)));
                }else{

                    showErr("您输入的金额已经超过目标金额",1);
                }
                }
