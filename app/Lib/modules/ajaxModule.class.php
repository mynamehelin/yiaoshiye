...
class ajaxModule extends BaseModule
{
	...
	{		
		...
		if($r!="")
		{
			...
			elseif($r=="nend")
			{
				$condition.=" and d.end_time - ".NOW_TIME." < ".(3*24*3600)." and d.end_time - ".NOW_TIME." > 0 ";  //三天就要结束
			}
			elseif($r=="classic")
			{
...
//		}
		...
		if($price>0){
			if($price==1){
				$orderby.=" support_amount desc";
				$param_new['price']=2;
 			}elseif($price==2){
				$orderby.=" support_amount asc";
				$param_new['price']=1;
 			}
			...
			if($focus ==1){
				$orderby.=" focus_count desc";
  			}elseif($focus ==2){
				$orderby.=" focus_count asc";
  			}
  		}elseif($time>0){
			if($time ==1){
			...
			if($cp ==1){
				$orderby.=" (support_amount+virtual_price)/limit_price desc";
  			}else{
				$orderby.=" (support_amount+virtual_price)/limit_price asc";
  			}
 		}else{
 			$orderby ="  d.begin_time desc ";
 		}
...
//		var_dump($orderby);
	...
	public function send_addMoney_succee(){
		
		$mobile = addslashes(htmlspecialchars(trim($_REQUEST['mobile'])));
		$money = addslashes(htmlspecialchars(trim($_REQUEST['money'])));
		$username = addslashes(htmlspecialchars(trim($_REQUEST['username'])));
		send_addMoney_sms($mobile,$money,$username);
		//ajax_return($money);		
	}
	*/
	...
	public function get_carry_fee(){
		...
		ajax_return(array('status'=>1,'fee'=>$fee));
	}
	/*
	 * 验证支付密码
	 */
