...
class accountModule extends BaseModule
{
	...
	public function index()
	{
		...
		$order_list = $GLOBALS['db']->getAll("select do.* from ".DB_PREFIX."deal_order as do left join ".DB_PREFIX."deal as de on de.id=do.deal_id where $condition and do.user_id = ".intval($GLOBALS['user_info']['id'])."  order by do.create_time desc limit ".$limit);
		...
		$order_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_order as do left join ".DB_PREFIX."deal as de on de.id=do.deal_id where $condition and do.user_id = ".intval($GLOBALS['user_info']['id']));
 		$order_sum = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_order as do left join ".DB_PREFIX."deal as de on de.id=do.deal_id where  do.user_id = ".intval($GLOBALS['user_info']['id']));
	...
	public function project()
	{
		...
		if($deal_status>0){
			switch($deal_status){
				...
				//进行中
				case 2:
				$condition .=" and is_effect=1 and is_success=0 and begin_time<".NOW_TIME." and  end_time>".NOW_TIME." ";
				break;
				//已成功
				case 3:
				$condition .= " and is_success=1 and is_effect=1 ";
				break;
				//已失败
				case 4:
	...
	public function set_repay(){
		...
		if($_REQUEST['type'] ==1)
			$order_info = $GLOBALS['db']->getRow("select d.*,i.stock_value as investment_stock_value from ".DB_PREFIX."deal_order as d left join ".DB_PREFIX."investment_list as i on i.id = d.invest_id  where d.id = ".$order_id." and d.order_status = 3 and d.is_refund = 0 and d.invest_id >0 order by d.create_time desc ");
		else
		...
		if($deal_info['type']==1){
			//项目金额
			$order_info['stock_value'] =number_format($deal_info['limit_price'],2);
			$order_info['money'] =$v['total_price'];
			//用户所占股份
			$order_info['user_stock']= ($deal_info['money']/$deal_info['limit_price'])*$deal_info['transfer_share'];
		}
	...
	public function record_pay(){
  	...
  	public function get_leader_list(){
		...
		if($page==0)
			$page = 1;
		$limit = (($page-1)*$page_size).",".$page_size;
  		$investor_list=$GLOBALS['db']->getAll("select invest.*,u.user_name,u.user_level from  ".DB_PREFIX."investment_list as invest left join ".DB_PREFIX."user as u on u.id=invest.user_id where invest.type=1 and invest.deal_id=$deal_id order by invest.id desc limit $limit ");
	...
	public function money_inchange()
	{
		...
		$GLOBALS['tmpl']->display("account_money_inchange.html");
			...
			public function money_below()
	{

		$GLOBALS['tmpl']->display("account_money_below.html");
	}
	...
	public function export_support_0($page = 1)
	{
		...
		if($support_list)
		{
	    	...
	    	foreach($support_list as $k=>$v)
			{
				...
				$order_value['delivery_fee'] = '"' . iconv('utf-8','gbk',round($v['delivery_fee'])."元") . '"';
				$order_value['share_fee'] = '"' . iconv('utf-8','gbk',round($v['share_fee'])."元") . '"';
	...
	public function FHJRmonyPAY()
	{
		file_put_contents("text1.txt",$_POST,FILE_APPEND);
	
		$payid           =   $_POST['payid'];//支付id
		
		$Godpay_id       =   $_POST['orderid'];//订单号 
		$sumb             =   $_POST['money'];//支付钱数
     ...
     // echo $ise->getLastSql();die;
	  if($ise==1){
		  
		$money = $GLOBALS['db']->query("update ".DB_PREFIX."user set money= money + {$sumb} where id = ".$user_id); //账户加钱
		if($money){
			header("location: http://www.yiaoshiye.com/index.php?ctl=account&act=record");die;	
		}
		
                 }

	 // echo"<script>alert('随便写点什么');history.go(-4);</script>";
