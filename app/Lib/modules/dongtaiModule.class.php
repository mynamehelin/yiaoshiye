...
class dongtaiModule extends BaseModule
{
...
//友情链接
        ...
        $article_shang= $GLOBALS['db']->getRow("select * from ".DB_PREFIX."article where id<".$article['id']." order by id desc limit 0,1") ;
        $article_xia= $GLOBALS['db']->getRow("select * from ".DB_PREFIX."article where id>".$article['id']." order by id desc limit 0,1") ;
        ...
        $GLOBALS['tmpl']->display("dongtai_index.html");
    }
}