...
function do_login_user()
{
	...
	if($.trim($("#user_login_form").find("input[name='user_pwd']").val())=="")
	{
		...
		if($.trim($("#user_login_form").find("input[name='adm_verify']").val())=="")
	{
		$.showErr("请输入验证码",function(){
			
			$("#user_login_form").find("input[name='adm_verify']").val("");
			$("#user_login_form").find("input[name='adm_verify']").focus();
...
function show_pop_login()
{
	...
	$.weeboxs.open(APP_ROOT+"/index.php?ctl=ajax&act=login", {
...
function resetWindowBox(){
	...
	var main_height=$(window).height() - $("#J_footer").outerHeight() - $("#").outerHeight();
	var box_height=$("#J_wrap").outerHeight();
	if($("#J_wrap").outerHeight() + $("#J_footer").outerHeight() + $("#").outerHeight() < $(window).height())
	{	
...
function show_pop_region(){
	$.weeboxs.open(APP_ROOT+"/index.php?ctl=ajax&act=region", {boxid:'pop_region',contentType:'ajax',showButton:false, showCancel:false, showOk:false,title:'选择地区',width:600,type:'wee'});
}
