			...
			<div class="ft-links <?php if ($this->_var['links']['type'] == 1): ?>ft-imglinks<?php endif; ?>">
				<div class="g_links <?php if ($this->_var['links']['type'] == 1): ?>g_imglinks<?php endif; ?>">
					...
					<div class="g_links_text f_l">
						<?php $_from = $this->_var['links']['list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('k', 'g_links');$this->_foreach['g_linkss'] = array('total' => count($_from), 'iteration' => 0);
...
?>
							...
							<a href="<?php echo $this->_var['g_links']['url']; ?>" target="_blank" <?php if ($this->_foreach['g_linkss']['iteration'] % 7 == 0): ?>class="last"<?php endif; ?>><img src="<?php echo $this->_var['g_links']['img']; ?>" alt="<?php echo $this->_var['g_links']['name']; ?>" width=160 height=74></a>
 							<?php endif; ?>
							<?php endif; ?>
...
echo $k['name']($k['v']);
?></div>
						<div class="qq">项目官方群:604126972</div>
						<div>工作时间:周一至周六<?php 
$k = array (
  'name' => 'app_conf',
  'v' => 'WORK_TIME',
	...
	<ul>
		...
		<li class="ewm_box">
			<a href="javascript:void(0);">
				<div class="sidebox_ewm"><img src="<?php echo $this->_var['TMPL']; ?>/images/sidebar_img/ewm.png">二维码</div>
				<div class="sidebox_hide sidebox_ewm_hide">
...
?>&site=qq&menu=yes" target="_blank">
				<div class="sidebox"><img src="<?php echo $this->_var['TMPL']; ?>/images/sidebar_img/qq.png">QQ咨询</div>
			</a>
		</li>
		<?php endif; ?>
...
?></div>
		...
		<li class="link_box" id="link_box">
			<a href="javascript:void(0);">
				<div class="sidebox_link tc"><i class="icon iconfont">&#xe603;</i></div>
				<div class="sidebox_hide sidebox_link_hide muxfb_global_tip" id="sidebox_link_hide">
...
<script type="text/javascript">
	...
	function sidebarFun(){
		$("#sidebar ul li").hover(function(){
			$(this).find(".sidebox").stop().animate({"width":"150px"},200).css("background","#3d9eeb")
		},function(){
			$(this).find(".sidebox").stop().animate({"width":"38px"},200).css("background","#939393")	
		});
		...
		$(window).scroll(function(){
			...
			if($("#sidebar").data("expanded")!=lastRmenuStatus){
				...
				if(lastRmenuStatus){
					$("#sidebar .sidetop").animate({"height":"38px"},200);
				}else{
					$("#sidebar .sidetop").animate({"height":"0"},200);
				}
			}
		});
		// 用户留言
