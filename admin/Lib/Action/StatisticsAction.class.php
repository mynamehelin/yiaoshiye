...
class StatisticsAction extends CommonAction{
	...
	public function usernum_info(){
		...
		if(trim($_REQUEST['time'])){
			$condtion = " and  (FROM_UNIXTIME(a.create_time + 28800, '%Y-%m-%d') = '$time')";
		}
		...
		$sql_str = "select  
		FROM_UNIXTIME(a.create_time , '%Y-%m-%d %H:%i:%S') as 时间 ,
		u.user_name as 用户名,
    ...
    public function money_total() 
    {
		...
		$sql_str = "select  
		FROM_UNIXTIME(do.create_time + 28800, '%Y-%m-%d') as 时间 ,
		count(*) as 支持人数, 
		sum(do.total_price) as 筹款总额,
		sum(if(do.is_success =1,do.total_price,0)) as 成功筹款,		
		sum(if(do.is_success =0,do.total_price,0)) as 失败筹款,	
		(select sum(money) from ".DB_PREFIX."deal_pay_log where  FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') in (".date_in($map['start_time'],$map['end_time']).") group by FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') ) as 已发放筹款,		
		...
		if( isset($map['start_time']) && $map['start_time'] <> '' && isset($map['end_time']) && $map['end_time'] <> ''){
			$sql_str .= " and FROM_UNIXTIME(do.create_time + 28800, '%Y-%m-%d') in (".date_in($map['start_time'],$map['end_time']).")";
		}
		$sql_str .= "  group by FROM_UNIXTIME(do.create_time + 28800, '%Y-%m-%d') ";
	...
	public function money_info(){
		...
		$time=trim($_REQUEST['time']);
		if(trim($_REQUEST['time'])){
			$condtion = " and  (FROM_UNIXTIME(do.create_time + 28800, '%Y-%m-%d') = '$time')";
		}
    ...
    public function hasback_total() 
    {
		...
		$sql_str = "select FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') as 时间, sum(if(repay_time>0 ,1,0)) as 已回报人数,sum(if(repay_time>0 , 1,0)) as 已回报项目, sum(if(repay_time='', 1,0)) as 待回报项目 
		from ".DB_PREFIX."deal_order  where order_status =3 and type = 0 and is_success =1 ";
		
		//日期期间使用in形式,以确保能正常使用到索引
		
		if( isset($map['start_time']) && $map['start_time'] <> '' && isset($map['end_time']) && $map['end_time'] <> ''){
			$sql_str .= " and FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') in (".date_in($map['start_time'],$map['end_time']).")";
		}
		$sql_str .= "  group by FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') ";
    ...
    public function overdue_total() 
    {
		...
		$sql_str = "select FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') as 回报时间, sum(if(is_refund =0,1,0)) as 逾期未回报项目数
		from ".DB_PREFIX."deal_order where order_status =3 and type = 0 and is_success =1";
    ...
    public function investors_total() 
    {
		...
		$sql_str = "select FROM_UNIXTIME(il.create_time + 28800, '%Y-%m-%d') as 时间, count( il.user_id) as 投资人总数, sum(if(u.is_investor=1, 1,0)) as 投资人, sum(if(u.is_investor=2,1,0)) as 投资机构 , sum(il.money) as 认投金额 , sum(if(il.investor_money_status=3,il.money,0)) as 成功融资金额
		from  ".DB_PREFIX."investment_list  as il left join ".DB_PREFIX."user as u on u.id = il.user_id where";
	
		//日期期间使用in形式,以确保能正常使用到索引
		if( isset($map['start_time']) && $map['start_time'] <> '' && isset($map['end_time']) && $map['end_time'] <> ''){
			$sql_str .= " FROM_UNIXTIME(il.create_time + 28800, '%Y-%m-%d') in (".date_in($map['start_time'],$map['end_time']).")";
		}
		$sql_str .= "  group by FROM_UNIXTIME(il.create_time + 28800, '%Y-%m-%d') ";
	...
	public function investors_info(){
		...
		$time=trim($_REQUEST['time']);
		if(trim($_REQUEST['time'])){
			$condtion = " (FROM_UNIXTIME(il.create_time + 28800, '%Y-%m-%d') = '$time')";
		}
    ...
    public function financing_amount_total() 
    {
	    ...
	    sum((select sum(total_price) from ".DB_PREFIX."deal_order where is_refund =0 and order_status =3 and type = 0 and deal_id = d.id )) as 待发放筹款,		
	...
	public function financing_amount_info(){
		...
		(select sum(total_price) from ".DB_PREFIX."deal_order where is_refund =0 and order_status =3 and type = 0 and deal_id = d.id ) as 待发放筹款,
    ...
    public function breach_convention_total() 
    {
		...
		$sql_str = "select FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') as 时间, count(user_id) as 违约人数, sum(money) as 违约金额
	...
	//	print_r(strtotime('2015-04-25'));exit;
		$sql_str = "select FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') as 时间, sum(money) as 成功充值总额
	...
	//	print_r(strtotime('2015-04-25'));exit;
		$sql_str = "select FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') as 时间, count(*) as 人次, sum(if(is_pay>0,money,0)) as 成功提现总额,  sum(money) as 申请提现总额
		from ".DB_PREFIX."user_refund where ";
		
		//日期期间使用in形式,以确保能正常使用到索引
		
		if( isset($map['start_time']) && $map['start_time'] <> '' && isset($map['end_time']) && $map['end_time'] <> ''){
			$sql_str .= "  FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') in (".date_in($map['start_time'],$map['end_time']).")";
		}
		$sql_str .= "  group by FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') ";
		$model = D();	
		$voList = $this->_Sql_list($model, $sql_str,'时间', false);
    ...
    public function money_carry_bank_info() 
    {
		...
		$time=trim($_REQUEST['time']);
		if(trim($_REQUEST['time'])){
			$condtion = "  (FROM_UNIXTIME(a.create_time + 28800, '%Y-%m-%d') = '$time')";
		}
    ...
    public function user_total() 
    {
		...
		$sql_str = "select FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') as 时间,count(*) as 注册人数, sum(if(is_investor=0,1,0)) as 普通会员, sum(if(is_investor=1,1,0)) as 投资人, sum(if(is_investor=2,1,0)) as 投资机构人
	...
	public function user_info(){
		...
		$time=trim($_REQUEST['time']);
		if(trim($_REQUEST['time'])){
			$condtion = " and  (FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') = '$time')";
		}
		...
		$sql_str = "select  
		FROM_UNIXTIME(create_time, '%Y-%m-%d %H:%i:%S') as 注册时间 ,
		user_name as 会员名称, 
		email as 会员邮箱, 
		mobile as 手机号,
		money as 会员余额,
...
//		var_dump($list);exit;
	...
	public function export_csv_usernum_total($page = 1){
		...
		$sql_str = "select FROM_UNIXTIME(create_time , '%Y-%m-%d') as 时间, count(DISTINCT user_id) as 支持人数, sum(total_price) as 支持总额 
		from ".DB_PREFIX."deal_order  where order_status =3 and type = 0 ";
	...
	public function export_csv_money_total($page = 1){
		...
		$sql_str = "select  
		FROM_UNIXTIME(do.create_time + 28800, '%Y-%m-%d') as sj ,
		count(*) as zcrs, 
		sum(do.total_price) as ckze,
		sum(if(do.is_success =1,do.total_price,0)) as cgck,		
		sum(if(do.is_success =0,do.total_price,0)) as sbck,	
		sum((select sum(money) from ".DB_PREFIX."deal_pay_log where deal_id = do.deal_id )) as yffck款,		
		
		sum(if(d.pay_radio >0,d.support_amount*d.pay_radio,d.support_amount*".app_conf("PAY_RADIO").")) as khdyj,			
	...
	public function export_csv_hasback_total($page = 1){
		...
		$sql_str = "select FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') as sj, sum(if(repay_time>0 ,1,0)) as yhbrs,sum(if(repay_time>0 , 1,0)) as yhbxm, sum(if(repay_time='', 1,0)) as dhbxm 
		from ".DB_PREFIX."deal_order  where order_status =3 and type = 0 and is_success =1 ";
	...
	public function export_csv_overdue_total($page = 1){
		...
		$sql_str = "select FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') as hbsj, sum(if(is_refund =0,1,0)) as yqwhbxms
		from ".DB_PREFIX."deal_order where order_status =3 and type = 0 and is_success =1";
...
//		var_dump($list);exit;
	...
	public function export_csv_investors_total($page = 1){
		...
		$sql_str = "select FROM_UNIXTIME(il.create_time + 28800, '%Y-%m-%d') as sj, count(il.user_id) as tzje, sum(if(u.is_investor=1, 1,0)) as tzr, sum(if(u.is_investor=2,1,0)) as jgtz, sum(il.money) as rtje , sum(if(il.investor_money_status=3,il.money,0)) as cglzje
		from  ".DB_PREFIX."investment_list  as il left join ".DB_PREFIX."user as u on u.id = il.user_id where";
...
//		var_dump($list);exit;
	...
	public function export_csv_financing_amount_total($page = 1){
	    ...
	    sum((select sum(total_price) from ".DB_PREFIX."deal_order where is_refund =0 and order_status =3 and type = 0 and deal_id = d.id )) as dffck,		
	...
	public function export_csv_financing_amount_info($page = 1){
		...
		(select sum(total_price) from ".DB_PREFIX."deal_order where is_refund =0 and order_status =3 and type = 0 and deal_id = d.id ) as dffck,
...
//		var_dump($list);exit;
	...
	public function export_csv_breach_convention_total($page = 1){
		...
		$sql_str = "select FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') as sj, count(user_id) as wyrs, sum(money) as wyje
		from ".DB_PREFIX."investment_list  where investor_money_status =4  ";
	...
	public function export_csv_money_inchange_total($page = 1){
		...
		$sql_str = "select FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') as sj, sum(money) as cgczze
		from ".DB_PREFIX."payment_notice  where deal_name ='' and  is_paid = 1 ";
	...
	//	print_r(strtotime('2015-04-25'));exit;
		$sql_str = "select FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') as sj, count(*) as rc, sum(if(is_pay>0,money,0)) as cgtxje,  sum(money) as sqtxze
		from ".DB_PREFIX."user_refund where ";
	...
	public function export_csv_money_carry_bank_info($page = 1){
		...
		$sql_str = "select  
		FROM_UNIXTIME(a.create_time, '%Y-%m-%d %H:%i:%S') as sj ,
		u.user_name as hymc, 
		a.money as txje, 
	...
	public function export_csv_user_total($page = 1){
		...
		$sql_str = "select FROM_UNIXTIME(create_time + 28800, '%Y-%m-%d') as sj,count(*) as zcrs, sum(if(is_investor=0,1,0)) as pthy, sum(if(is_investor=1,1,0)) as tzr, sum(if(is_investor=2,1,0)) as tzjgr
		from ".DB_PREFIX."user where is_effect = 1 ";
	...
	public function export_csv_user_info($page = 1){
		...
		$sql_str = "select  
		FROM_UNIXTIME(create_time, '%Y-%m-%d %H:%i:%S') as zcsj ,
		user_name as hymc, 
		email as hyyx, 
		mobile as sjh,
		money as hyye,
