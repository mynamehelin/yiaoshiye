...
class SmsAction extends CommonAction{
	...
	public function send_demo()
	{
		...
		/*分红短信*/
	public function send_charing()
	{
		//echo $_POST['nowmobile'];
		//exit;
		//$test_mobile = $_REQUEST['test_mobile'];
		require_once APP_ROOT_PATH."system/utils/es_sms.php";
		$sms = new sms_sender();

		$result = $sms->sendSms($_POST['nowmobile'],l("SHARING_SMS"));
		if($result['status'])
		{
			$this->success(l("SEND_SUCCESS"),1);
		}
	...
	public function send_vote()
	{
		...
		$result = $sms->sendSms($_POST['nowmobile'],l("VOTE_SMS"));
		if($result['status'])
		{
			$this->success(l("SEND_SUCCESS"),1);
		}
